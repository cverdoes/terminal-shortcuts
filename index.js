...
function readLines(filepath) {
	try {
		if (pathExists(filepath)) {
			var fileContents = fs.readFileSync(shortcutsFilePath).toString();
			...
			console.error('No shortcuts file found (' + shortcutsFilePath + ')');
			return [];
		}
	} catch (er) {
		console.error('Failed to read shortcuts file (' + shortcutsFilePath + ')');
		return [];
	}
}
